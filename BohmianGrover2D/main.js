...
function evolveCurrentSimulation(dt,sign=1){const signedDt=sign*dt;mixingSign=Math.sign(signedDt)||1;particleStep(signedDt,false);waveRk4(signedDt);}
function freeStep(dt){evolveCurrentSimulation(dt);simTime+=Math.abs(dt);simSteps++;}
...
function advanceOperation(){if(!activeSegment||paused)return 0;let advanced=0;
  if(activeSegment.type==='free'){
    ...
    for(let i=0;i<maxSteps&&activeSegment.elapsed<activeSegment.duration-1e-12;i++){
      ...
      for(const target of completedTargets){activateSimulation(simulationForTarget(target));evolveCurrentSimulation(dt,activeSegment.sign??1);}
...
function opInverse(){
  ...
  startOperation([{type:'free',duration:MIX_TIME,sign:-1,circuitStage:3,label:'3a. Diffuser: inverse mixer A† (−H)'}],3);
}
...
function opFull(){
  ...
  startOperation([
    ...
    {type:'free',duration:MIX_TIME,sign:-1,circuitStage:3,label:'3a. Diffuser: inverse mixer A† (−H)'},
...
function resetSimulation(update=true){operationQueue=[];activeSegment=null;stageIndex=0;simTime=0;simSteps=0;mixingSign=1;gateFlash=0;paused=false;dom.pause.textContent='Pause';for(const simulation of simulations){activateSimulation(simulation);uploadInitialWave();rebuildParticles();normalizeWave();simulation.diagnostics=null;}restoreSelectedSimulation();lastDiagnostics=null;updateProgress();syncButtons();updateStageStatus();if(update)updateDiagnostics(true);}
